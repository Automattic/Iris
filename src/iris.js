	...
	*/
	...
	var Iris = {
		...
		_addTooltip: function() {
			var self = this;
			$("<span class='iris-tooltip' />").text(self.options.tooltip).appendTo( self.controls.squareDrag );
			self.controls.square.on( 'mousedown.iris', self, function( event ){
				if ( event.which === 1 ) // left click
					self.controls.square.off('mousedown.iris').find('.iris-tooltip').remove();
			});
		},
		show: function() {
			this.picker.show();
